...
def execute_integration(activation: dict, queue: Any = None) -> None:
    """Land an approved candidate; see `claude_integration`. No model call.

    The identity is read here, at call time, and passed in, so the extracted
    module does not import this one and a patched AGENT_IDENTITY is honoured.
    """
    claude_integration.execute_integration(
        activation, queue, actor=AGENT_IDENTITY
    )
